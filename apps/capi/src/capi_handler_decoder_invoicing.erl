...
decode_payment_operation_failure({failure, Failure}, Context) ->
    case capi_auth:get_consumer(capi_handler_utils:get_auth_context(Context)) of
        client ->
...
-spec make_invoice_and_token(capi_handler_encoder:encode_data(), binary(), woody_context:ctx()) ->
    capi_handler_decoder_utils:decode_data().
make_invoice_and_token(Invoice, PartyID, WoodyContext) ->
    #{
